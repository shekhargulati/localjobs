...
public class LocalJobsController {
	...
	@RequestMapping("/localjobs/{jobId}")
	...
	public ResponseEntity<String> allJobsNear(@RequestParam("latitude")double latitude, 
		...
		for (Job localJob : jobs) {
			DistanceResponse response = googleDistanceClient.findDirections(localJob.getLocation(), new double[]{latitude,longitude});
			LocalJobWithDistance linkedinJobWithDistance = new LocalJobWithDistance(localJob,response.rows[0].elements[0].distance,response.rows[0].elements[0].duration);
	...
	@RequestMapping("/localjobs/near/{skill}")
	...
	@RequestMapping("/localjobs/near/{skill}/{location}")
	public ResponseEntity<String> allJobsNearToLocationWithSkill(@PathVariable("skill")String skill, @PathVariable("location")String location) throws Exception{
		HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
